...
# Initialize Let's Encrypt certificates for standard ports (80/443) - Podman version
# This script sets up SSL certificates using Certbot with HTTP-01 challenge on port 80
...
DATA_PATH="./certbot"
NGINX_CONF_PATH="./nginx/conf.d-standard-ports"

echo "### Initializing Let's Encrypt for $DOMAIN (standard ports 80/443) - Podman ###"
echo ""

# Validate domain
if [ "$DOMAIN" = "your-domain.com" ]; then
    echo "Error: Please provide your domain name as the first argument"
    echo "Usage: ./init-letsencrypt-standard-ports-podman.sh your-domain.com your-email@example.com [staging]"
    exit 1
fi
...
mkdir -p "$DATA_PATH/conf/live/$DOMAIN"
podman-compose -f $COMPOSE_FILE run --rm --entrypoint "\
...
# Start nginx
echo "### Starting nginx ###"
podman-compose -f $COMPOSE_FILE up -d nginx
echo
...
# Run certbot with HTTP-01 challenge on port 80
podman-compose -f $COMPOSE_FILE run --rm --entrypoint "\
